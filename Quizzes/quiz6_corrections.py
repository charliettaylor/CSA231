...
#     from main() and the total and blank line count from get_questions_banks()
#     '''
#     print("-" * 57)
...
# Part 2
# I changed a couple things, which were the way I counted skipped lines in
# get_question_bank() and how missing words were dealt with in
# find_most_similar(). When counting skipped lines, it would only check for
# new line characters.
...
def get_question_bank(fileName: str):
    ...
    for idx, question in enumerate(cleaned):
        for word in question:
            if not word.isalpha():
                cleaned.remove(cleaned[idx])
                # add counter for when lines are removed
                blankLines += 1
...
def find_most_similar(questions: list, vectors: dict):
    ...
    for idx in range(len(questions)):
        ...
        for q in range(1, 5):
            testWord = questions[idx][0]
            compare = questions[idx][q]
            # None is passed in when testWord = 'Three' and compare = 'three'
            # because neither exist in the word vectors created by james.py
            # no handler for bad input
            if testWord in vectors.keys() and compare in vectors.keys():
                score = james.cosine_simularity(vectors.get(testWord),
                                                vectors.get(compare))
            if score > previous:
                mostSimilar = compare
